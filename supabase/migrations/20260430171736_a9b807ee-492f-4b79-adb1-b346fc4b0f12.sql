...
BEGIN
  ...
  IF p_ip_address IS NOT NULL AND p_ip_address <> '' THEN
    ...
    IF v_count > 30 THEN
      ...
      VALUES (v_token, p_ip_address, p_user_agent, NULLIF(v_caller_email, ''), 'rate_limited');
  ...
  WHERE e.signing_token = v_token;

  IF v_envelope_id IS NULL THEN
    INSERT INTO public.envelope_access_log
      (signing_token, ip_address, user_agent, caller_email, outcome)
    VALUES (v_token, p_ip_address, p_user_agent, NULLIF(v_caller_email, ''), 'not_found');
    RETURN NULL;
  END IF;
  ...
  THEN
    ...
    VALUES (v_envelope_id, v_token, p_ip_address, p_user_agent, v_caller_email, 'email_mismatch');
  ...
  INSERT INTO public.envelope_access_log
    (envelope_id, signing_token, ip_address, user_agent, caller_email, outcome)
  VALUES (v_envelope_id, v_token, p_ip_address, p_user_agent, NULLIF(v_caller_email, ''), 'allowed');
